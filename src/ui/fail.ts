const getFailIconSvgInnerHtml = (failText: string) =>
  `<svg
...
</svg><b style="color:white;">${failText}</b>`

export const createFailText = (failText: string) => {
  const svgFailIcon = window.document.createElement('div')
  svgFailIcon.style.cssText = 'display: flex;align-items: center;'
  svgFailIcon.innerHTML = getFailIconSvgInnerHtml(failText)
  return svgFailIcon
}
